...
public class Server {
	...
	private static boolean validatePacket(DatagramPacket p) throws Exception {
		...
		if (ip.contains(ipAddress))
			isValid = true;
			System.out.println("Validate ip: true");
		else
			return false;*/
		...
		parser.parse(is);
		String[] data = handler.getRequest();
		String id = data[3];
		...
		if (inPassword.equals(data[2])){
			isValid = true;
			System.out.println("Validate: Password was a match!");
		}else isValid = false;
	...
	private static void sendReply(Notices notices, InetAddress ip, int port, String uuid) {
		...
		try {
			...
			if (size > 0 && size <= 5) {
				...
				writer.writeTag("plc", HOSPITAL_ID, true);
		...
		try {
			...
			while (running) {
				...
				if (packetIsOk) {
					...
					switch (msgType) {
					case REQUEST:
						System.out.println("Beginning handling of request");
